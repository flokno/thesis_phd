...
\begin{align}
	E_v [n] 
		...
		\equiv F[n] + \int \d^3 x ~ v ({\bf x}) n ({\bf x})~.
	\label{eq:dft.En}
\end{align}
...
\begin{align}
	E_v[n] < E_v [n'] \quad \text{for} \quad n \neq n'~.
\end{align}
...
\begin{align}
	E^{\rm BO} ({\bf R}) 
		= E[n] + \sum_{I < J} \frac{Z_{I} Z_{J} e^{2}}{\left|{\b R}_{I}-{\bf R}_{J}\right|}~.
	\label{eq:E^BO}
\end{align}
...
\begin{align}
		\frac{\d}{\d {\bf R}_I} E[n]
			...
			+ \underset{II}{\underbrace{
					\int \d^{3} x ~ \frac{\delta E[n]}{\delta n(\boldsymbol{x})} \frac{\partial n(\boldsymbol{x})}{\partial \boldsymbol{R}_{I}}
				}}~,
...
\begin{align}
	...
	- \sum_{I \neq J} \frac{Z_I Z_J e^2 ({\b R}_{I}-{\bf R}_{J})}{
		\left\lvert {\b R}_{I}-{\bf R}_{J} \right\rvert^3}~,
