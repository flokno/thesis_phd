...
\end{align}
\TODO{Is anything of this necessary besides defining the eigenvectors once and introducing $s = (b, \b q)$?}
\REM{Alternative}
\begin{align}
	{D}_{i \alpha, j \beta} (\b q) 	
		&= \frac{1}{N_{\rm sc}} 
...
\begin{align}
	D_{i \alpha, j \beta} (- \b q) 
		& = D_{i \alpha, j \beta}^\ast (\b q)~,\text{ and} \\
	e_{b, i \alpha} (- \b q)
		&= e^\ast_{b, i \alpha} (\b q)~.
\end{align}
With the help of Eq.\,\eqref{eq:D_ij(q)}, the dynamical matrix $\mathrm D_{IJ}$ for the full system can be written as
\begin{align}
...
Using the combined index $s = (\b q, b)$, the notation becomes similar to the non-periodic case with $3N_{\rm sc} N_{\rm uc}$ degrees of freedom, only that the eigenvectors $\b e_{s = (\b q, b)}$ can be complex-valued instead of strictly real:
\begin{align}
	D_{I \alpha, J \beta}
		&= \sum_s \omega^{2}_{s \vphantom I} \, \fD e_{s, I \alpha} e^\ast_{s, J \beta}~.
\end{align}

\newthought{In terms of complex amplitudes $a^{(\dagger)}_b (\b q)$}, we define\marginnote{The sign convention enforces that $$u_b^\ast (\b q) = u_b (- \b q)~,$$ so that the displacements $u_I$ are always real.}
\begin{subequations}
