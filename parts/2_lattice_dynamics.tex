...
For the set of commensurate $\b q$-points, the dynamical matrix is given by
\begin{align}
{\rm D}_{i \alpha, j \beta} (\b q_{\bf m}) 	
	&= %\frac{N}{N_{\rm sc}} 
	\sum_{\b L \in V_{\rm sc}} {\rm e}^{- \im \b q_{\bf m} \cdot \b L} {\rm D}_{i \b 0 \alpha, j \b L \beta}~,
	\label{eq:D^S(q).2}
\end{align}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=.32\textwidth]{./notes/2_sc.pdf}
	\hfill
	\includegraphics[width=.32\textwidth]{./notes/3_sc.pdf}
	\hfill
	\includegraphics[width=.32\textwidth]{./notes/4_sc.pdf}
	\caption{Depiction of square supercells with lattice points in the range $[-0.5 L, 0.5 L)$ (bullets $\bullet$), and extended lattice points at the supercell boundary (empty bullets $\circ$), where $L$ is the length of the supercell.}
	\label{fig:sketch_supercells}
\end{figure}

\subsubsection{Interpolation to non-commensurate q-points}
The definition of the dynamical matrix in Eq.\,\eqref{eq:D^S(q).2} was formulated for commensurate $\b q$-points $\set{\b q_{\b m}}$,~i.\,e.,~those given in terms of Eq.\,\eqref{eq:q_m}. Evaluating Eq.\,\eqref{eq:D^S(q).2} for any other value of $\b q$ will however yield a non-vanishing matrix.
