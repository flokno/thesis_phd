...
\begin{align}
	{\rm D}_{i \alpha, j \beta} (\b q) 
		= \frac{1}{N} \sum_{\b L} {\rm e}^{\im \b q \cdot \b L} {\rm D}_{i \b 0 \alpha, j \b L \beta}
	\label{eq:D(q)}
\end{align}
...
\begin{align}
	{\rm D}_{i \alpha, j \beta} (\b q) 
		&= \frac{1}{N} 
			...
			\right)
	\label{eq:D^S(q).1} \\
	&= \frac{1}{N_{\rm sc}} \sum_{\b L \in V_{\rm sc}} {\rm e}^{\im \b q \cdot \b L} {\rm D}_{i \b 0 \alpha, j \b L \beta}~,
	\label{eq:D^S(q).2}
\end{align}
...
\end{align}
since only for these $\b q$ the summands in the second sum of Eq.\,\eqref{eq:D^S(q).1} interfere constructively, and for all other $\b q$ the entire summand vanishes in the limit $1/N \to 0$.
