...
\marginnote{\REM{Probably define $\sqrt 2 \sin \omega t)$ so that $\braket{u_s^2} = A^2_s$ and $E=\sum_s \omega_s^2 A_s^2$?}}
\begin{align}
	u_s (t) 
	...
	p_s (t) 
		...
		\equiv \sqrt{\frac{\omega_s}{2}} \im \left[ \D a_s (t) - \fD a_s (t) \right]
\end{align}
with
\begin{align}
	a_s (t) 
...
The Hamiltonian in terms of the complex amplitudes $a^{(\dagger)}_s$ reads\marginnote{The relation to quantum mechanics is achieved by replacing the complex numbers $a^{(\dagger)}_s$ with the operators $\hat{a}^{(\dagger)}_s$ fulfilling the canonical commutation relations $$\left[ \fD{\hat{a}}_{\fP s}, \D{\hat a}_{s'} \right] = \delta_{ss'}~.$$}
\begin{align}
	\mathcal{H}(\b a, \D{\b a})
		&= \halb \sum_s \omega_s \left[ \D a_s\fD  a_s + \fD a_s \D a_s \right] \\
		&= \sum_s \omega^2_s A^2_s
\end{align}
