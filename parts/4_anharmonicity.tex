...
\begin{figure}
	...
	\caption{
		Boltzmann transport calculation of thermal conductivity at room temperature including third- and fourth-order contributions for 34 zinc blende and rocksalt compounds from Ref.\,\cite{Xia2020}. $\kappa^3$: Boltzmann transport with three-phonon scattering rates computed from third-order terms in an expansion of the potential-energy surface, cf. Eq.\,\eqref{eq:Gamma_s}. $\kappa^3_{\rm SCPH}$: Boltzmann transport with three-phonon scattering and additionally accounting for phonon-frequency renormalization via self-consistent phonon theory~\cite{Xia2018}. These approximations are compared with $\kappa^{(3+4)}_{\rm SCPH}$,~i.\,e.,~the highest available level of theory where both frequency renormalization and four-phonon scattering are additionally included in the computation of thermal conductivity. The data points give the relative change with respect to $\kappa^{(3+4)}_{\rm SCPH}$,~i.\,e.,~1 means perfect agreement between different levels of theory.
	}
	\label{fig:anh.bte}
\end{figure}

The figure shows a comparison of three different levels of perturbation theory employed in that work: i) $\kappa^3$ with three-phonon scattering computed from third-order force constants, ii) $\kappa^3_{\rm SCPH}$, which additionally includes the effect of phonon frequency renormalization at finite temperatures, and iii) $\kappa^{(3+4)}_{\rm SCPH}$, where furthermore four-phonon scattering from a fourth-order expansion of the potential-energy surface is accounted for~\cite{Feng2016, Xia2018}. We show the results from lower-level theory, $\kappa^3$ and $\kappa^3_{\rm SCPH}$, in comparison with the highest-available level, $\kappa^{(3+4)}_{\rm SCPH}$, by computing their ratio, and compare with one-shot $\sigmaA$ values from our anharmonicity screening~\cite{Knoop2020}.\footnote{Xia and coworkers investigated 19~rock~salt and 17~zincblende compounds~\cite{Xia2020}, from which 18 (16) were included in our screening~\cite{Knoop2020}.}
